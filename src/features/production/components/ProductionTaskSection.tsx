...
export function ProductionTaskSection({ variant = "page", layout = "full", limit }: ProductionTaskSectionProps) {
  ...
  const productionStats = useMemo(() => {
    ...
    const unclaimed = tasks.filter((t) => t.status === "Pending" && t.assigned_to == null).length;
    const mine = tasks.filter((t) => t.assigned_to === user.id).length;
    const mineInProgress = tasks.filter(
  ...
  const productionStatCards = useMemo((): ProductionStatCard[] => {
    if (!productionStats) return [];
    return [
      {
        title: "Production Queue",
        value: String(productionStats.total),
        change: "Active production tasks",
        icon: Factory,
        department: "production",
        accentColor: "border-l-orange-500",
      },
      {
        ...
        value: String(productionStats.unclaimed),
        change: "Available to start",
        icon: Clock,
        department: "production",
        accentColor: "border-l-orange-500",
      },
      {
        title: "My Print Jobs",
        value: String(productionStats.mine),
      ...
      {
        title: "In progress",
        value: String(productionStats.mineInProgress),
        change: "You are actively printing",
        icon: Cog,
        highlight: productionStats.mineInProgress > 0,
        department: "production",
        accentColor: "border-l-orange-500",
      },
    ];
  ...
  const statsCards = productionStats ? (
    <>
      {productionStatCards.map((stat) => (
      <Card key={stat.title} className={cn("bg-card shadow-sm border-l-4", stat.accentColor, stat.highlight && "border-orange-500/60")}>
  ...
  return (
    <div className="space-y-6">
      {showHeader && <div className={embedded ? "" : "px-2"}>
        {embedded ? (
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">Line-item production queue — start and mark ready per task.</p>
          </div>
        ) : (
          <>
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Production Tasks</h1>
                ...
                <CardHeader className="pb-4 px-4 sm:px-6">
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      {task.assigned_to == null ? "---" : isMine ? "You" : assigneeName ?? "---"}
                    </p>
                  </div>
                </CardHeader>
